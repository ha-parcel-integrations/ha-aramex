"""Aramex parcel tracker custom component for Home Assistant."""
...
from .api import AramexApiClient
...
@dataclass
class AramexData:
    """Runtime data attached to the Aramex config entry."""

    client: AramexApiClient
    coordinator: AramexCoordinator


type AramexConfigEntry = ConfigEntry[AramexData]


async def async_setup_entry(hass: HomeAssistant, entry: AramexConfigEntry) -> bool:
    """Set up Aramex from a config entry."""
    # No auth: Aramex tracking is public, so the HA-managed session is fine.
    client = AramexApiClient(async_get_clientsession(hass))
    ...
    entry.runtime_data = AramexData(client=client, coordinator=coordinator)
...
async def _async_options_updated(
    hass: HomeAssistant, entry: AramexConfigEntry
) -> None:
    """Apply changed options by refreshing the coordinator."""
    await entry.runtime_data.coordinator.async_request_refresh()


async def async_unload_entry(hass: HomeAssistant, entry: AramexConfigEntry) -> bool:
    """Unload the Aramex config entry."""
    if not await hass.config_entries.async_unload_platforms(entry, PLATFORMS):
        return False
    # Services are shared by all country hubs. Keep them while another hub is
    # loaded, so unloading AU does not break automation services for NZ.
    if not any(
