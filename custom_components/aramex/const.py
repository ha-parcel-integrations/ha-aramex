"""Constants for the Aramex parcel tracker integration."""
...
# The AU/NZ surface exposes only a public tracking page and scan history.
...
# ``TRACKING_API_URLS`` are the country-specific consumer endpoints;
# ``TRACKING_URL_BASES`` are the human-facing tracking pages surfaced on each
# parcel's ``url`` field, confirmed deep-linkable with `?l=<label>` (e.g.
# https://www.aramex.com.au/tools/track?l=39134643030). Document what you
# know about the endpoint right here — auth, rate limits, response content
# type, and how an unknown tracking code is signalled. The next person to
# touch this file has no other source in this repo — the full write-up lives
# in your own private research notes, not in this repo.
TRACKING_API_URLS = {
    "AU": "https://www.aramex.com.au/umbraco/api/TrackingApi/GetTrackingData",
    "NZ": "https://www.aramex.co.nz/umbraco/api/TrackingApi/GetTrackingData",
}
TRACKING_URL_BASES = {
    "AU": "https://www.aramex.com.au/tools/track",
    "NZ": "https://www.aramex.co.nz/tools/track",
}

# Local timezone per country, used only to turn the payload's date-only
# ``DeliveryETADate`` into an all-day ``planned_from``/``planned_to`` window.
TIMEZONES = {"AU": "Australia/Sydney", "NZ": "Pacific/Auckland"}
