"""Coordinator for the Aramex parcel tracker integration.
...
from .api import AramexApiClient, AramexApiError
from .const import (
    CONF_COUNTRY,
...
class AramexCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    def _tracked(self) -> list[dict[str, str]]:
    ...
    def _country(self) -> str:
        """Return this hub's configured country dataset."""
        return self.config_entry.options.get(CONF_COUNTRY, "AU")
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        parcels = self._tracked()
        codes = [item[CONF_TRACKING_CODE] for item in parcels]
        ...
        parcels_to_fetch = [
            item for item in parcels
            if item[CONF_TRACKING_CODE] not in self._delivered_codes
        ]
        codes_to_fetch = [item[CONF_TRACKING_CODE] for item in parcels_to_fetch]

        results = await asyncio.gather(
            *(
                self._client.async_get_parcel(item[CONF_TRACKING_CODE], self._country)
                for item in parcels_to_fetch
            ),
        ...
        saw_429 = False
        for item, result in zip(parcels_to_fetch, results):
            code = item[CONF_TRACKING_CODE]
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (AramexApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                if (
                    isinstance(result, AramexApiError)
                    and result.status_code == 429
                ):
                    saw_429 = True
                    if result.retry_after is not None:
                        retry_afters.append(result.retry_after)
                _LOGGER.warning("Aramex fetch failed: %s", result)
            ...
            # its key.
            sanitized = sanitize_tracking_payload(result)
            self._raw_cache[code] = sanitized
            raws_by_code[code] = sanitized
        ...
        if saw_429:
            ...
            raise UpdateFailed(
                "Aramex rate-limited (429)", retry_after=retry_after
            )
        self._consecutive_429 = 0

        if codes_to_fetch and errors == len(codes_to_fetch) and not raws_by_code:
            raise UpdateFailed("Aramex unreachable for all tracked parcels")
