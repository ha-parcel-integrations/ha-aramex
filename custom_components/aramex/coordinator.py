...
class AramexCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        results = await asyncio.gather(
            *(self._client.async_get_parcel(item[CONF_TRACKING_CODE], self._country) for item in parcels),
        ...
        saw_429 = False
        for item, result in zip(parcels, results):
            ...
            if result is None:
                ...
                raws.append(self._raw_cache.get(code) or {"tracking_code": code, "country": self._country})
                continue
