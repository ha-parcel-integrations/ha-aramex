"""Sensor platform for the Aramex parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up Aramex sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        AramexIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            AramexParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(AramexNextDeliverySensor(coordinator, entry))
    entities.append(AramexDeliveredParcelsSensor(coordinator, entry))
    entities.append(AramexLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class AramexIncomingParcelsSensor(
    CoordinatorEntity[AramexCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                AramexParcelSensor(self.coordinator, self._entry, barcode)
...
class AramexParcelSensor(CoordinatorEntity[AramexCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Aramex parcel."""
    ...
    def __init__(
        self, coordinator: AramexCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class AramexNextDeliverySensor(
    CoordinatorEntity[AramexCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: AramexCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class AramexDeliveredParcelsSensor(
    CoordinatorEntity[AramexCoordinator], SensorEntity
):
    """Recently delivered tracked Aramex parcels."""
...
class AramexLastUpdateSensor(
    CoordinatorEntity[AramexCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Aramex was last polled successfully."""
