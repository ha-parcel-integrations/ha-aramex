"""Config flow for the Aramex parcel tracker integration."""
...
_HUB_SCHEMA = vol.Schema(
    {
        vol.Required(CONF_COUNTRY): selector.SelectSelector(
            selector.SelectSelectorConfig(
                # Selector values are lower-case translation keys. Store the
                # carrier/API country code in its canonical upper-case form.
                options=["au", "nz"],
...
def normalize_tracking_code(value: str) -> str:
    """Return an Aramex label with surrounding whitespace removed only."""
...
def _clean_tracking_codes(values: list[str] | str | None) -> list[str]:
...
class AramexConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle the UI-driven configuration flow for the Aramex integration."""
    ...
    ) -> AramexOptionsFlowHandler:
        """Return the options flow handler."""
        return AramexOptionsFlowHandler()

    async def async_step_user(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        """Create one hub for an AU or NZ tracking dataset.

        Each country is a separate hub, which makes it possible to track AU
        and NZ parcels side by side. Parcels inherit the country's dataset
        from their hub.
        """
...
class AramexOptionsFlowHandler(OptionsFlow):
