"""Tests for the Aramex coordinator: fetching, caching and events.
...
from custom_components.aramex.api import AramexApiError
...
from custom_components.aramex.coordinator import AramexCoordinator
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    ...
    sample["result"]["Scans"][-1]["Status"] = "PPP"
    sample["result"]["Scans"][-1]["StatusDescription"] = "In transit"
    return sample
...
async def test_update_merges_multiple_parcels(hass):
    ...
    client = AsyncMock()
    client.async_get_parcel.side_effect = lambda code, _country: (
        active_sample() if code == ACTIVE_CODE else delivered_sample()
    )
    coordinator = AramexCoordinator(hass, client, entry)
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = AramexApiError("HTTP 500")
    await coordinator._async_update_data()  # error -> cached raw reused
    assert len(coordinator.delivered) == 1


async def test_cache_strips_carrier_pii_and_failure_log_hides_label(hass, caplog):
    entry = _entry_with([{CONF_TRACKING_CODE: DELIVERED_CODE}])
    entry.add_to_hass(hass)
    sample = delivered_sample()
    sample["result"]["AddressOnParcel"] = "1 Private Street"
    sample["result"]["Signature"] = "private signature"
    sample["result"]["Scans"][0]["Name"] = "Private Person"
    client = AsyncMock()
    client.async_get_parcel.return_value = sample
    coordinator = AramexCoordinator(hass, client, entry)
    await coordinator._async_update_data()

    cached = coordinator._raw_cache[DELIVERED_CODE]
    assert "AddressOnParcel" not in str(cached)
    assert "Signature" not in str(cached)
    assert "Name" not in str(cached)

    client.async_get_parcel.side_effect = AramexApiError("HTTP 500")
    await coordinator._async_update_data()
    assert DELIVERED_CODE not in caplog.text
...
async def test_update_fetches_parcels_concurrently(hass):
    ...
    async def _slow_fetch(code, _country):
...
async def test_delivered_code_skipped_from_fetch(hass):
    ...
    client.async_get_parcel.assert_called_once_with(ACTIVE_CODE, coordinator._country)
...
async def test_fires_registered_event_for_new_parcel(hass):
    ...
    client.async_get_parcel.side_effect = lambda code, _country: active_sample(code)
    await coordinator._async_update_data()
    await hass.async_block_till_done()
...
async def test_no_delivery_time_events_without_an_evidenced_eta(hass):
    entry = _entry_with([{CONF_TRACKING_CODE: ACTIVE_CODE}])
    entry.add_to_hass(hass)
    client = AsyncMock()
    coordinator = AramexCoordinator(hass, client, entry)
