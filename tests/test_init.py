"""Tests for Aramex setup and unload."""
...
async def test_setup_and_unload(hass):
    ...
    incoming = hass.states.get("sensor.aramex_au_incoming_parcels")
...
async def test_setup_retries_when_first_refresh_fails(hass):
    ...
    with patch(
        "custom_components.aramex.api.AramexApiClient.async_get_parcel",
        new=AsyncMock(side_effect=AramexApiError("Aramex unreachable")),
    ):
...
async def test_per_parcel_sensor_spawn_and_remove(hass):
    ...
    with patch("custom_components.aramex.api.AramexApiClient.async_get_parcel", new=mock):
        assert await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()
...
async def test_options_update_applies_live_without_reload(hass):
    ...
    with patch("custom_components.aramex.api.AramexApiClient.async_get_parcel", new=mock):
        ...
        mock.side_effect = lambda code, _country: _sample(code)
