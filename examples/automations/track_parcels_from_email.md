# Automatic parcel tracking from e-mail (IMAP → Aramex)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Aramex tracking code, and register it with `aramex.track_parcel` — fully automatic, no extra custom component required.

Aramex is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `aramex.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ aramex.track_parcel
                                     │
...
- This integration, with the `aramex.track_parcel` action available (field `tracking_code`).
...
```
(?<![0-9A-Za-z])(?:MP[0-9A-Za-z]{6,}|[0-9]{9,})
```

Two shapes are covered: an **`MP…`** label (e.g. `MP0085494934`, the format seen on real captured AU/NZ parcels) and a **bare numeric AWB** (e.g. `39134643030`, as used in the carrier's own `?l=` tracking deep link). The integration itself accepts any code matching `^[A-Za-z0-9]{6,20}$`, but matching *that* broadly against e-mail text would hit every order number in the mail — so the example anchors on known shapes instead. **If your mails carry a different prefix, add it to the alternation** (`(?:MP[0-9A-Za-z]{6,}|[0-9]{9,}|YOURPREFIX[0-9A-Za-z]{6,})`); the AI fallback will surface unknown formats in the meantime.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@aramexshipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: MP0085494999","initial":true,"folder":"INBOX","username":"test"}'
```

Then `aramex.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
